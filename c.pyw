...
for color in colors.split('\n'):
	sp = color.split(' ')
	bg=sp[1]
	if color=='#000000 Black 纯黑':
		bg='White'
	else:
		bg=sp[1]
	try:
		Label(text=color, bg=bg).grid(row=int(i/colcut),column=i%colcut, sticky=W+E+N+S)
	except:
		print('颜色错误：',color)
		Label(text='颜色错误：'+color,bg='#ffffff').grid(row=int(i/colcut),column=i%colcut, sticky=W+E+N+S)
	i += 1
root.mainloop()